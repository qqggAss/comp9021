...
def f(arg_for_seed, nb_of_elements, max_element):
    ...
    for i in range(len(new_L)):
        end_digit = new_L[i] % 10
        num_dict[end_digit].append(new_L[i])

    for digit, numbers in num_dict.items():
        num_dict[digit] = sorted(numbers, key=lambda x: (-len(str(x)), new_L.index(x)))

    # 将排序后的列表按照列表长度和最后一位数字的大小进行排序
    R = sorted(num_dict.values(), key=lambda x: (-len(x), -int(str(x[0])[-1]) if x else 0))
