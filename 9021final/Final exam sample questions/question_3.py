...
def f(m, n, direction):
    ...
    str_m = str(m)
    str_n = str(n)
    length = max(len(str_m), len(str_n))
    if len(str_m) < length:
        str_m = (length - len(str_m)) * '0' + str_m
    if len(str_n) < length:
        str_n = (length - len(str_n)) * '0' + str_n
    if direction == 'L':
        result = []
        result_length = []
        for i in range(length):
            if i == 0:
                sum = int(str_m) + int(str_n)
                result.append((str_m, str_n, sum))
                result_length.append(len(str(sum)))
            else:
                ...
                result.append((str_m, str_n, sum))
                result_length.append(len(str(sum)))
        rs = max(result_length)
        format = f'0{rs}d'
        for i in range(len(result)):
            print(f'{result[i][0]} plus {result[i][1]} equals {result[i][2]:{format}}')
        ...
        for i in range(length):
            if i == 0:
                ...
                str_m = str_m[-1] + str_m[:length - 1]
                str_n = str_n[-1] + str_n[:length - 1]
                sum = int(str_m) + int(str_n)
                result.append((str_m, str_n, sum))
                result_length.append(len(str(sum)))
        rs = max(result_length)
